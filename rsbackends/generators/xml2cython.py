...
def main(argv=None):
    ...
    try:
        try:
            opts, args = getopt.getopt(argv[1:], "ho:l:f:i:s:V",
                                       ["help", "output", "location-filter",
                                        "function-name-filter",
                                        "input-file-filter", "included-symbols", "--version"])
    ...
    for o, a in opts:
        if o in ("-h", "--help"):
            ...
            lfilter_str = a
        elif o in ("-f", "--function-name-filter"): # only ONE can be used
            ffilter_str = a
    ...
    # Input file filter - we add the symbols it contains
    if ifilter:
        a = open(ifilter, 'r')
        try:
